...
export default function EarningsScreen() {
  const { earnings, earningsPeriod, setEarningsPeriod, bookings, worker } = useWorker();
  ...
  const chartData = React.useMemo(() => {
    ...
    if (earningsPeriod === 'today') {
      ...
      completed.forEach(b => {
        ...
        hours[h] = (hours[h] || 0) + b.net_amount;
      });
    ...
    if (earningsPeriod === 'week') {
      ...
      completed.forEach(b => {
        ...
        if (diff < 7) {
          ...
          data[dayName] = (data[dayName] || 0) + b.net_amount;
        }
      });
    ...
    completed.forEach(b => {
      ...
      if (diff < 28) {
        ...
        data[wk] = (data[wk] || 0) + b.net_amount;
      }
    });
    return weeks.map(w => ({ label: w, amount: data[w] }));
  }, [bookings, earningsPeriod]);

  // Skill breakdown data
  const skillData = earnings.by_skill.map(s => {
    const meta = SKILL_CATEGORIES.find(c => c.key === s.category);
    return { ...s, emoji: meta?.emoji ?? '🔧', color: meta?.color ?? '#059669' };
  });
