...
interface InteractiveMapProps {
  ...
  workers: ProviderProfile[];
  onSelectWorker?: (worker: ProviderProfile) => void;
}
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      if (!leafletMap.current) {
        leafletMap.current = L.map(mapRef.current).setView([userLoc.lat, userLoc.lng], 13);
      ...
      workers.forEach((w) => {
        const markerColor = w.featured ? '#F5A623' : '#0B3D66';
        const providerMarker = L.circleMarker([w.lat, w.lng], {
          radius: 8,
        ...
        const popupContent = `
          <div style="font-family: system-ui; text-align: center; padding: 2px;">
            <b style="color: #0B3D66; font-size: 13px;">${w.name}</b><br/>
            <span style="font-size: 11px; color: #475569;">${w.role}</span><br/>
            <span style="font-size: 11px; color: #16A34A; font-weight: bold;">₹${w.hourly_rate}/hr</span><br/>
            <button id="book-btn-${w.id}" style="margin-top: 6px; background: #072A4A; color: white; border: none; padding: 4px 10px; border-radius: 6px; font-size: 11px; font-weight: bold; cursor: pointer;">
              View & Book
            </button>
          </div>
        `;
