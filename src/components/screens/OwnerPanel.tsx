...
import { Provider, OWNER_PHONES, PRIMARY_SUPER_OWNER } from '../../lib/types';
...
export default function OwnerPanel({ onClose }: { onClose: () => void }) {
  const { providers, refreshProviders, user, showToast, bookings } = useApp();
  ...
  const totalRevenue = bookings.reduce((s, b) => s + b.commission_amount, 0);
