...
export default function WorkerScreen({ onJobPosted }: { onJobPosted?: () => void }) {
  const { user, providers, bookings, addWorkerProfile, updateBookingStatus, showToast } = useApp();
  ...
  const handlePostService = (e: React.FormEvent) => {
    ...
    const newProvider: Provider = {
      ...
      name: name.trim(),
      category: category,
      hourly_rate: parseInt(hourlyRate, 10) || 400,
      phone: phone.trim(),
      description: description.trim() || `${category} Specialist available for instant booking`,
      is_online: true,
      rating: 5.0,
      review_count: 1,
      featured: true,
  ...
  const totalGross = completedJobs.reduce((sum, b) => sum + b.total_amount, 0);
                ...
                {bookings.map(b => (
                  <div key={b.id} style={{ background: 'white', borderRadius: 18, padding: 16, border: '1px solid #E2E8F0', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #F1F5F9', paddingTop: 10 }}>
                      <div>
                        <span style={{ fontSize: 13, fontWeight: 900, color: '#0F172A' }}>₹{b.total_amount}</span>
                        <span style={{ fontSize: 10, color: '#64748B', marginLeft: 6 }}>(Net ₹{b.total_amount - (b.commission_amount || 0)})</span>
                      </div>
